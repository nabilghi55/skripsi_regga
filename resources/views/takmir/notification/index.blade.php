...
@section('title', 'Notifikasi Masjid')

@section('content')
<!-- Back Header -->
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; flex-wrap: wrap; gap: 10px;">
    <a href="{{ route('takmir.dashboard') }}" class="form-header-back" style="margin-bottom: 0;">
...
<div class="content-card" style="padding: 24px;">
    <h3 style="font-size: 15px; font-weight: 800; color: var(--text-dark); margin-bottom: 20px; text-transform: uppercase; border-bottom: 1px solid var(--border-color); padding-bottom: 10px; display: flex; align-items: center; gap: 8px;">
        ...
        Kotak Masuk Notifikasi Takmir
    </h3>

    @if(!$masjid)
        <div style="color: var(--text-muted); text-align: center; padding: 40px 0; font-size: 13px;">
            Akun Anda belum terasosiasi dengan Masjid Binaan. Hubungi Admin SIKJ CMM.
        </div>
    @else
        <div class="notification-inbox-list">
            ...
            @empty
                <div style="color: var(--text-muted); text-align: center; padding: 40px 0; font-size: 13px;">
                    Belum ada notifikasi masuk untuk Masjid Anda.
                </div>
            @endforelse
        </div>
...
function markAsRead(notifId) {
    const url = "{{ route('takmir.markNotifRead', ':id') }}".replace(':id', notifId);
