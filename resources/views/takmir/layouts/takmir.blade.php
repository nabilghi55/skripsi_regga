...
<body style="background-color: var(--bg-main);">
    ...
    <header class="takmir-navbar">
        <div class="takmir-navbar-container">
            ...
            <nav class="takmir-nav-links">
                @php
                    $unreadNotifCount = 0;
                    if (Auth::user() && Auth::user()->masjid) {
                        $unreadNotifCount = \App\Models\Notification::where('masjid_id', Auth::user()->masjid->id)->whereNull('read_at')->count();
                    }
                @endphp
                ...
                <a href="{{ route('takmir.riwayat') }}" class="takmir-nav-link {{ Request::routeIs('takmir.riwayat') ? 'active' : '' }}">Riwayat</a>
                <a href="{{ route('takmir.notification.index') }}" class="takmir-nav-link {{ Request::routeIs('takmir.notification.index') ? 'active' : '' }}">
                    Notifikasi
    ...
    <nav class="mobile-bottom-nav">
        ...
        <a href="{{ route('takmir.riwayat') }}" class="mobile-nav-item {{ Request::routeIs('takmir.riwayat') ? 'active' : '' }}">
            <span class="mobile-nav-icon">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            </span>
            <span>Riwayat</span>
        </a>
        <a href="{{ route('takmir.notification.index') }}" class="mobile-nav-item {{ Request::routeIs('takmir.notification.index') ? 'active' : '' }}">
