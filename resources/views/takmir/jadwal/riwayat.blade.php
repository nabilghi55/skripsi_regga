...
<div class="content-card">
    ...
    <!-- Filter Form -->
    <form action="{{ route('takmir.riwayat') }}" method="GET" style="margin-bottom: 24px;">
        <div style="display: grid; grid-template-columns: 3fr 1fr; gap: 12px; align-items: end;">
            ...
            <div style="display: flex; gap: 8px;">
                ...
                @if($search)
                    <a href="{{ route('takmir.riwayat') }}" class="btn btn-secondary" style="height: 42px; width: 42px; display: inline-flex; align-items: center; justify-content: center; padding: 0; text-decoration: none;">
            ...
            <tbody>
                @forelse($jadwals as $index => $j)
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <button onclick="openSaranModal('{{ route('takmir.jadwal.saran', $j->id) }}', '{{ $j->tanggal->translatedFormat('d F Y') }}', '{{ addslashes($j->catatan_saran_takmir) }}')" class="btn btn-primary btn-detail-sm" style="padding: 6px 10px; font-size: 11px; font-weight: bold; width: auto; height: auto; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; background-color: var(--border-color); color: var(--text-dark); border-color: var(--border-color);">
                ...
                @empty
                    <tr>
                        <td colspan="6" style="text-align: center; color: var(--text-muted); padding: 30px 10px;">
                            Tidak ada riwayat jadwal khotbah yang ditemukan.
                        </td>
                    </tr>
                @endforelse
            </tbody>
...
document.getElementById('saran-close-btn').addEventListener('click', function() {
    document.getElementById('saran-modal').style.display = 'none';
});

// Close modal when clicking outside card
document.getElementById('saran-modal').addEventListener('click', function(e) {
    if (e.target === this) {
